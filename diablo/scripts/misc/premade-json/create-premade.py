...
for next_key in interval_keys[1:]:
    ...
    if DEBUG:
        print("[*] DEBUG: KEYS: ({}, {}); Start {}; End {}; Inc {}".format(
            current_key, next_key, start_tps, end_tps, increment))
...
full_transactions = total_transactions * \
    int(CONFIG["secondaries"]) * int(CONFIG["threads"])
...
if DEBUG:
    print("[*] DEBUG: \n\t [*] Secondary {}\n\t [*] Thread {}\n\t [*] Interval {}".format(
        len(premade_workload), len(premade_workload[0]), len(premade_workload[0][0])))
